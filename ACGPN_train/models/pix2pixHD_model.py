...
class Pix2PixHDModel(BaseModel):
    ...
    def forward(self,label,pre_clothes_mask,img_fore,clothes_mask,clothes,all_clothes_label,real_image,pose,mask, person_lm, cloth_lm, cloth_rep, mesh, dense, densearms):
        ...
        if self.opt.clothrepG2:
            cloth_G2_in = cloth_rep
        else:
            cloth_G2_in = clothes

        if self.opt.clothlmg2:
            G2_in=torch.cat([pre_clothes_mask,cloth_G2_in,masked_label,pose,self.gen_noise(shape)],1)
        elif self.opt.denseplus:
            G2_in=torch.cat([pre_clothes_mask,cloth_G2_in,masked_label,pose, dense,self.gen_noise(shape)],1)
        else:
            G2_in = torch.cat([pre_clothes_mask, cloth_G2_in, masked_label, pose, self.gen_noise(shape)], 1)

        if self.opt.noopenpose:
            G2_in = torch.cat([pre_clothes_mask, cloth_G2_in, masked_label, dense, self.gen_noise(shape)], 1)
